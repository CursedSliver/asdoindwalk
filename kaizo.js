...
function addLoc(str, str2) {
	locStrings[str] = str2??str;
}
...
Game.registerMod("Kaizo Cookies", { 
	...
	header: function() {
		...
		decay.clickWork = 0.3; //the base amount of work that each click does
		...
		decay.wrinklerApproachPow = 0.6; //the more it is, the faster wrinklers approach the big cookie with increased decay
		decay.purityToRequiredHaltPow = 0.7; //purity is raised to this power when calculating its effect on required halt
		decay.accToRequiredHaltPow = 0.5; //acceleration is raised to this when calculating its power on required halt
		decay.accToRequiredHaltMinimum = 2; //minimum required for the effect to take place
        ...
        decay.getRequiredHalt = function() {
			...
			r *= Math.pow(Math.min(Math.max(decay.acceleration - decay.accToRequiredHaltMinimum + 1, 1), decay.accToRequiredHaltMaximum), decay.accToRequiredHaltPow);
		...
		decay.saveHaltValues = function() {
			let str = '';
			for (let i in decay.halts) {
				if (!(decay.halts[i].halt || decay.halts[i].overtime)) { continue; }
		...
		decay.exhaust = function() {
			decay.exhaustion = decay.exhaustionBegin * decay.exhaustionBeginMult;
			decay.fatigue = 0;
		...
		decay.onInf = function() {
			...
			if (decay.prefs.ascendOnInf || Game.ascensionMode == 42069) { decay.forceAscend(true); Game.Notify(loc('Infinite decay'), loc('Excess decay caused a forced ascension without gaining any prestige or heavenly chips.'), [21, 25], Game.fps * 3600 * 24 * 365, false, 1); decay.refreshAll(1); }
		}
		...
		Game.registerHook('reset', function(hard) { decay.onAscending(); if (kaizoCookies.paused) { kaizoCookies.togglePause(); } if (hard) { decay.wipeSave(); } });
		...
		decay.notifs = {
			...
			shinySoulEffect: {
				...
				desc: 'Upon claiming a shiny wrinkler\'s soul, a golden cookie will spawn, yielding a Reflective blessing: giving you cookies scaling with your current CpS (including any buffs that may be active), for up to doubling your bank! You might be able to do something with this information...',
		...
		decay.triggerNotif = function(key, bypass) {
			...
			if (!decay.hasEncounteredNotif) { Game.Notify('Options', 'Look into the options menu for accessibility options (especially if you play on a touchpad), the ability to pause the game, and to replay these informational notifications!', 0, 1e21, false, true); }
		...
		addLoc('Assistance option; allows you to automatically click wrinklers you are hovering over by scrolling, but click speed is capped to 9 clicks per second.');
		...
		decay.getPrefButtons = function() {
			...
			str += decay.writePrefButton('scrollWrinklers', 'scrollWrinklersButton', loc('Easy wrinklers')+' ON', loc('Easy wrinklers')+' OFF')+'<label>('+loc('Assistance option; allows you to automatically click wrinklers you are hovering over by scrolling, but click speed is capped to 9 clicks per second.')+')</label><br>';
		...
		decay.setWidget = function() {
			...
			if (Game.specialTab == 'dragon') { verticalPlacement = 0.75; } 
			else if (Game.specialTab == 'santa') { verticalPlacement = 0.8; }
			else if (Game.specialTab == 'utenglobe') { 
				if (decay.utenglobeTab == 0) { verticalPlacement = 0.72; }
				else if (decay.utenglobeTab == 1) { verticalPlacement = 0.65; }
			}
		...
		decay.setWrinklerApproach = function() {
			...
			if (Game.Has('Wrinklerspawn')) { mult *= 1.1; }
			if (Game.hasGod) {
				const godLvl = Game.hasGod('scorn');
				if (godLvl == 1) { mult *= (1 + 0.6); }
				else if (godLvl == 2) { mult *= (1 + 0.4); }
				else if (godLvl == 3) { mult *= (1 + 0.2); }
			}
			if (decay.challengeStatus('live1')) { mult *= 1/0.8; }
			return Math.min(Math.sqrt(Game.log10Cookies), (Game.Has('Legacy')?10:3)) * 0.0005 * (1 / mult) + 1 / Math.max(5, base * mult / Math.pow((Math.log(1 / Math.min(1, decay.gen)) / Math.log(decay.wrinklerApproachFactor)), decay.wrinklerApproachPow));
		};
		...
		decay.setWrinklerSpawnRate = function() {
			if (decay.gen >= decay.wrinklerSpawnThreshold || Game.cookiesEarned < decay.featureUnlockThresholds.wrinklers || Game.Has("Wrinkler doormat")) { return 0; }
			var base = 0;
			for (let i = 0; i < decay.wrinklerSpawnRateMapKeys.length; i++) {
				if (decay.wrinklerSpawnRateMapKeys[i] <= Game.log10Cookies) {
					continue;
				}
				//smooth interpolation
				const upper = decay.wrinklerSpawnRateMap[decay.wrinklerSpawnRateMapKeys[i]];
				const lower = decay.wrinklerSpawnRateMap[decay.wrinklerSpawnRateMapKeys[i - 1]];
				const diff = decay.wrinklerSpawnRateMapKeys[i] - decay.wrinklerSpawnRateMapKeys[i - 1];
				base = 1 - (upper * (diff - (decay.wrinklerSpawnRateMapKeys[i] - Game.log10Cookies)) / diff + lower * (diff - (Game.log10Cookies - decay.wrinklerSpawnRateMapKeys[i - 1])) / diff);
				//console.log(base);
				break;
			}

			var mult = 1;
			if (decay.gen > 1) { mult *= Math.pow(decay.gen, 0.1); }
		...
		decay.setWrinklerResistance = function() {
			...
			if (Game.Has('Santaic doom')) { 
				base *= 1 + Math.max(Game.santaLevel - 7, 0) * 0.04;
			}
			return base;
		};
		...
		decay.wrinklerMovement = new Crumbs.behavior(function() {
			const pos = Crumbs.h.rv(this.rad, 0, 108 + this.dist * 72); //108 pixels
		...
		decay.wrinklerAI = new Crumbs.behavior(function() {
			this.dist = Math.max(this.dist - (1 / Math.max((this.hurt - 10) / 10, 1)) * this.speedMult * (decay.wrinklerApproach / Game.fps) * (this.shiny?0.33:1), 0);
		});
		...
		decay.onWrinklerClick = function() {
			if (Game.hasBuff('Power surge') && decay.spendPowerClick()) {
				const shinyMult = (this.shiny?0.5:1);
				decay.damageWrinkler.call(this, 200 * shinyMult, true);
				let allWrinklers = Crumbs.getObjects('w');
				this.dist += 0.5;

				for (let i in allWrinklers) {
					if (allWrinklers[i] == this) { continue; }
					decay.damageWrinkler.call(allWrinklers[i], 36 * shinyMult);
					if (allWrinklers[i].size > 0) { decay.damageWrinkler.call(allWrinklers[i], 36 * shinyMult); }
					allWrinklers[i].dist += 0.2;
				}
				return;
			}
			decay.damageWrinkler.call(this, decay.wrinklerResistance * (this.shiny?0.33:1) * 1000 / Game.fps / Game.deltaTime);
		}
		...
		decay.spawnWrinklerLead = function() {
			...
			obj.size = randomFloor(Math.pow(Math.max(Game.log10Cookies * (1 + Math.random() * 0.5) - 8, 1), 0.4));
			if (rand > 95 && Game.log10Cookies > 48) { obj.size += 2; }
			if (rand > 90 && Game.log10Cookies > 48) { obj.size++; }
			if (rand > 60) { obj.size++; } 
			if (rand > 25 && Game.log10Cookies > 24) { obj.size++; }
		...
		decay.wSoulDeposit = new Crumbs.behavior(function(p) {
			if (this.y > Crumbs.scopedCanvas.left.l.height * (1 - Game.milkHd)) { 
			...
			if (this.inMilk > 2 * Game.fps) { decay.utenglobeStorage[(this.shiny?'shinyS':'s')+'oul'].deposit(1); this.die(); }
		});
		...
		decay.createSoulClaimAura = function(soul) {
			...
			Crumbs.spawn({
				...
				expandSpeed: ((s?512:384) + 10 * (soul.shiny?decay.shinySoulClaimAuraPower:decay.soulClaimAuraPower)) / Game.fps,
		...
		decay.updateSoulClaimAuraPowers = function() {
			if (decay.soulClaimAuraPower > 0) {
				decay.soulClaimAuraPower -= decay.times.sinceSoulClaim / Game.fps / 4;
			} 
			if (decay.shinySoulClaimAuraPower > 0) {
				decay.shinySoulClaimAuraPower -= decay.times.sinceSoulClaim / Game.fps / 4;
			}
		}
		Game.registerHook('logic', decay.updateSoulClaimAuraPowers);
		decay.saveMaterialCounts = function() {
			return decay.soulClaimCount + ',' + decay.shinySoulClaimCount;
		}
		...
		decay.removeAllWrinklerSouls = function() {
			let wrinklerSouls = Crumbs.getObjects('s');
		...
		addLoc('Shift-click to take out all of a material.');
		addLoc('Toggle this on to have released souls autonomously fly into the big cookie.');
		decay.utenglobeAutoClaimTooltip = function() { return '<div style="min-width: 200px; padding: 6px; text-align: center;">'+loc('Toggle this on to have released souls autonomously fly into the big cookie.')+'</div>'; }
		decay.utenglobeAutoClaim = false;
		...
		decay.allUtenglobeTabs = [
			{ 
				...
				details: function() {
					...
					return str + '<div style="width: 100%; margin-bottom: 2px; height: 6px;"><small>'+loc('Shift-click to take out all of a material.')+'</small></div>';
				}
			}, 
			{ 
				text: loc('upgrade'),
		...
		injectCSS(`.block.incomplete { font-size: 10px; border-color: #e0c020; box-shadow: 0px 0px 3px 0px #e0c020; }`);
		...
		decay.ugSoulCookieUpgradeCostMap = [1e7, 1e8, 1e9, 1e10, 1e12, 1e15, 1e19, 1e24, 1e30, 1e37, 1e45, 1e54];
		decay.tryBuyUtenglobeCookieUpgrade = function() {
			...
			Game.cookies -= decay.ugSoulCookieUpgradeCostMap[decay.utenglobeSoulCookieUpgradeCount];
		...
		decay.utenglobeStorage = {
			soul: new decay.utenglobeStorageItem('soul', 'Wrinkler soul', 10, [9, 3, kaizoCookies.images.custImg], function() { 
				...
				const secondButton = '<div class="block incomplete">' + loc('Unlock another <b>%1</b> achievements to raise maximum storage by another <b>%2</b> (Currently: <b>+%3</b>)', [Beautify((Math.floor((Game.AchievementsOwned + 60) / 130) + 1) * 130 - 60 - Game.AchievementsOwned), 2, Beautify(Math.min(Math.floor((Game.AchievementsOwned + 60) / 130), 5) * 2)]) + '</div>';
			...
			shinySoul: new decay.utenglobeStorageItem('shinySoul', 'Shiny wrinkler soul', 4, [10, 3, kaizoCookies.images.custImg], function() { 
				...
				const prompt = '<div class="block incomplete">' + loc('Complete <b>%1</b> more unshackled decay challenges to raise maximum storage by <b>%2</b> (Currently: <b>+%3</b>)', [Beautify(Math.pow(2, Math.ceil(Math.log2(decay.challengesCompleted + 1))) - 1 - decay.challengesCompleted), 1, Math.floor(Math.log2(decay.challengesCompleted + 1))]) + '</div>';
		...
		decay.purifyFromShimmer = function(obj) {
			...
			if (obj.spawnLead) {
				...
				decay.purifyAll(strength, 1 - Math.pow(0.99, strength), 10); decay.triggerNotif("pesudonat");
			}
		}
		decay.clickBCStop = function() {
			...
			for (let i in decay.offBrandFingers) {
				if (decay.offBrandFingers[i].bought) { offbrandAdd += 0.25; }
			}
			base *= offbrandAdd;
			...
			decay.stop(base * 1000 / Game.fps / Game.deltaTime, 'click');
		...
		this.changePledge = function () {
			...
			replaceDesc('Sacrificial rolling pins', 'The Elder Pledge is <b>twice</b> as cheap.<q>As its name suggests, it suffers so that everyone can live tomorrow.</q>');
			Game.Upgrades['One mind'].clickFunction = function () { return true; };
			Game.Upgrades['Elder Pact'].clickFunction = function () { return true; };
			replaceDesc('Elder Pledge', 'Purifies the decay, at least for a short while.<br>Price scales with highest CpS this ascension.<q>Incredible power lies within.</q>');
			...
			Game.Upgrades['Elder Pledge'].priceFunc = function () {
				return Game.cookiesPsRawHighest * 180 * (Game.Has('Sacrificial rolling pins') ? 0.5 : 1);
			}
		...
		decay.getCFChance = function() { 
			let chance = 0.5;
			if (decay.challengeStatus('combo1')) { chance += 0.1; }
		...
		this.reworkGarden = function() {
			...
			eval("M.computeEffs="+M.computeEffs.toString().replace("goldenClover') effs.goldenCookieFreq+=0.03*mult;","goldenClover') { effs.goldenCookieFreq+=0.03*mult; effs.goldenCookieEffDur*=1-0.015; effs.goldenCookieGain+=0.5; }"));
			...
			M.plants['goldenClover'].effsStr='<div class="green">&bull; '+loc("golden cookie frequency")+' +3%</div><div class="green">&bull; '+loc("golden cookie gains")+' +50%</div><div class="red">&bull; '+loc('golden cookie effect duration')+' -1.5%</div>';
			...
			M.plants['gildmillet'].effsStr='<div class="green">&bull; '+loc("golden cookie gains")+' +38.9%</div><div class="green">&bull; '+loc("golden cookie effect duration")+' +0.1%</div>';
		...
		this.reworkPantheon = function() {
			...
			addLoc('Wrinkler move <b>+%1%</b> slower.');
			addLoc('Wrath cookies replaces Golden cookies with %1% less decay.');
			temp.gods['scorn'].desc1='<span class="green">'+loc('Wrinkler move <b>+%1%</b> slower.', 60)+'</span> <span class="red">'+loc('Wrath cookies replaces Golden cookies with %1% less decay.', 50);
			temp.gods['scorn'].desc2='<span class="green">'+loc('Wrinkler move <b>+%1%</b> slower.', 40)+'</span> <span class="red">'+loc('Wrath cookies replaces Golden cookies with %1% less decay.', 33);
			temp.gods['scorn'].desc3='<span class="green">'+loc('Wrinkler move <b>+%1%</b> slower.', 20)+'</span> <span class="red">'+loc('Wrath cookies replaces Golden cookies with %1% less decay.', 20);
		...
		decay.checkGCUpgradeUnlocks = function() {
			if (Game.goldenClicksLocal>=1) { 
				...
				if (Game.goldenClicks>=3) { Game.Unlock('Lucky radar'); }
				if (Game.goldenClicks>=7) { Game.Unlock('Shimmering encapsulation'); }
				if (Game.goldenClicks>=17) { Game.Unlock('Immense flow'); }
			}
		...
		Game.Upgrades['Heavenly cookie stand'].basePrice = 1.111111111111111e8;
		...
		.replace(
			...
			`'<div id="prestigePowerDisplay" style="margin-top: 3px;">'+decay.getPrestigeLevelUnleashText()+((Game.heavenlyMult>=1)?(' '+loc("at x%1 effect and count", Beautify(Game.GetHeavenlyMultiplier(), 2))):'')+'</div>'`
		));
	 ...
	 const arr = ['Christmas tree biscuits','Snowflake biscuits','Snowman biscuits','Holly biscuits','Candy cane biscuits','Bell biscuits','Present biscuits'];
	  ...
	  for (let i = 0; i < 4; i++) { if (!Game.HasUnlocked(cookie) && !Game.Has(cookie)) { Game.Unlock(cookie); break; } else { cookie = choose(arr); if (i == 3) { cookie = ''; } } }
		...
		Game.Upgrades['Santa\'s dominion'].cost = Math.pow(17, 17);
		...
		*/
		...
		addLoc('Decay gains \"Acceleration\", which is an ever-increasing boost to decay propagation, faster with existing decay and slower with existing purity. Decay and wrinkler-related mechanics unlock much earlier than normal.<div class=\"line\"></div>Survive and make cookies in order to unlock certain upgrades. The more cookies you make, the more upgrades you unlock. Additional information and sub-challenges are available during the challenge mode in the Stats menu.');
		Game.ascensionModes[42069] = {name:'Unshackled decay',dname:loc("Unshackled decay"),desc:loc("Decay gains \"Acceleration\", which is an ever-increasing boost to decay propagation, faster with existing decay and slower with existing purity. <br>Decay and momentum unlock much earlier than normal. Normal permanent upgrade slots are disabled.<div class=\"line\"></div>Survive and make cookies in order to unlock certain upgrades. The more cookies you make, the more upgrades you unlock. Additional information and sub-challenges are available during the challenge mode in the Stats menu."),icon:[23,13,kaizoCookies.images.custImg]};
		...
		decay.challenge.prototype.save = function() {
			var str = this.complete + '^' + (this.cannotComplete?1:0);
		...
		decay.challenge.prototype.load = function(str) {
			...
			if (isv(str[1]) && str[1][0] != 'c') { this.cannotComplete = parseInt(str[1]); }
		...
		decay.challengeDisplay = function() {
			...
			for (let i in ch) {
				...
				if (ch[i].cannotComplete && Game.ascensionMode == 42069) { cannotCompleteWarning = true; str2 += ' cannotComplete'; } str2 += '"';
			...
			if (!str2) {
				str2 += '<div class="singleChallenge">' + loc('It seems that there\'s nothing here. Come back after getting some more heavenly upgrades! (Specifically, the Vial of challenges)') + '</div>';
			}
			if (hasPrereq) {
		...
		new decay.challenge('hc', loc('Bake <b>%1</b> with <b>no</b> normal upgrades <b>bought</b>.', Beautify(1e9)+loc(' cookies')), function() { return Game.cookiesEarned >= 1e9; }, loc('<b>Halved</b> flavored cookie cost')+'<br>'+loc('Hardcore'), decay.challengeUnlockModules.vial, { prereq: 'combo1', onCompletion: function() { Game.Win('Hardcore'); } });
		...
		new decay.challenge('combo2', loc('Get a <b>Click frenzy</b> and a <b>Frenzy</b> active simultaneously in the first <b>%1</b> of the run.', Game.sayTime(4.5 * 60 * Game.fps, -1)), function(c) { if (Game.TCount >= 4.5 * 60 * Game.fps) { c.cannotComplete = true; } return (Game.hasBuff('Click frenzy') && Game.hasBuff('Frenzy')); }, function(hide) { return loc('The Lucky day upgrade no longer resets on ascension') + '<br>' + loc('A <b>%1</b> %2 multiplier that gradually decreases with your current progress in a run', ['+' + Beautify(1818) + '%', 'click power']) + ' ' + loc('(does not contribute to future click power multiplier checks)') + (hide?'':(' '+loc('(Currently: <b>%1</b>)', '+'+Beautify(1818 / (Math.max(Game.log10Cookies - 12, 0) / 5 + 1))+'%'))) }, decay.challengeUnlockModules.vial, { onCompletion: function() { decay.triggerNotif('combos'); }, prereq: 'combo1' });
		...
		eval('Game.Reset='+Game.Reset.toString().replace(`Game.gainedPrestige=0;`, `Game.gainedPrestige=0; var hasLucky = false; if (Game.Has('Lucky day')) { hasLucky = true; }`).replace(`BeautifyAll();`, `if (!hard && hasLucky && decay.challengeStatus('combo2')) { Game.Upgrades['Lucky day'].earn(); } BeautifyAll();`));
		...
		new decay.challenge('combo4', loc('Get a click power multiplier of at least <b>x%1</b> in the first <b>%2</b> of the run.', [Beautify(50000), Game.sayTime(10 * 60 * Game.fps)]) + '<br>' + loc('(note: there is no way to stack Click frenzy and Dragonflight in this mod)'), function(c) { if (Game.TCount >= 600 * Game.fps) { c.cannotComplete = true; } return (Game.clickMult >= 50000); }, loc('Sacrificing the garden leaves the %1 seed in addition to Baker\'s Wheat.', 'Thumbcorn') + '<br>' + loc('Click frenzy from Force the Hand of Fate Grimoire spell chance <b>%1</b> --> <b>%2</b>', ['60%', '70%']), decay.challengeUnlockModules.vial, { prereq: 'combo3' });
		addLoc('Get a click power multiplier of at least <b>x%1</b> with only one buff active and without Santa\'s helpers. Then, obtain a Dragon harvest while Dragonflight is <b>not</b> active.');
		new decay.challenge('comboDragonCursor', loc('Get a click power multiplier of at least <b>x%1</b> with only one buff active and without Santa\'s helpers. Then, obtain a Dragon harvest while Dragonflight is <b>not</b> active.', [Beautify(1282)]), new decay.challengeChecker(function(t) { if (t.cpcValid && Game.hasBuff('Dragon Harvest')) { t.cpcValid = false; return true; } if (Game.clickMult >= 1282 && !Game.hasBuff('Dragonflight') && Game.buffCount() <= 1 && !Game.Has('Santa\'s helpers')) { t.cpcValid = true; } else { t.cpcValid = false; } return false; }, function() { return ''; }, function() { }, { cpcValid: false }), loc('All dragon auras cost <b>half</b> as much buildings to unlock'), decay.challengeUnlockModules.vial, { prereq: 'combo3' });
		...
		decay.recalcAccStats = function() {
			decay.accInc = 0.035 / Game.fps;
			decay.accBuffAsymptoteThreshold = decay.accBuffAsymptotePow / 6;
			decay.startingAcc = 1.2;
			if (Game.Has('Enchanted Permanent upgrade slot I')) { decay.startingAcc -= 0.01; }
			if (Game.Has('Enchanted Permanent upgrade slot II')) { decay.startingAcc -= 0.01; }
			if (Game.Has('Enchanted Permanent upgrade slot III')) { decay.startingAcc -= 0.01; }
			if (Game.Has('Enchanted Permanent upgrade slot IV')) { decay.startingAcc -= 0.01; }
			if (Game.Has('Enchanted Permanent upgrade slot V')) { decay.startingAcc -= 0.01; }
		...
		this.createAchievements=function(){//Adding the custom upgrades
			...
			this.achievements.push(new Game.Upgrade('Enchanted Permanent upgrade slot I',"Placing an upgrade in this slot will make its effects <b>permanent</b> across all playthroughs, <b>even in the Unshackled decay challenge mode</b>.<br>In addition, having this upgrade makes you start with <b>0.01</b> less acceleration for the Unshackled decay challenge mode.",	1,[0,0,kaizoCookies.images.custImg]));Game.last.pool='prestige';Game.last.iconFunction=function(){return Game.EnchantedPermanentSlotIcon(0);};Game.last.activateFunction=function(){Game.AssignEnchantedPermanentSlot(0);};
			...
			this.achievements.push(new Game.Upgrade('Enchanted Permanent upgrade slot II',"Placing an upgrade in this slot will make its effects <b>permanent</b> across all playthroughs, <b>even in the Unshackled decay challenge mode</b>.<br>In addition, having this upgrade makes you start with <b>0.01</b> less acceleration for the Unshackled decay challenge mode.",	200,[1,0,kaizoCookies.images.custImg]));Game.last.pool='prestige';Game.last.iconFunction=function(){return Game.EnchantedPermanentSlotIcon(1);};Game.last.activateFunction=function(){Game.AssignEnchantedPermanentSlot(1);};
			...
			this.achievements.push(new Game.Upgrade('Enchanted Permanent upgrade slot IV',"Placing an upgrade in this slot will make its effects <b>permanent</b> across all playthroughs, <b>even in the Unshackled decay challenge mode</b>.<br>In addition, having this upgrade makes you start with <b>0.01</b> less acceleration for the Unshackled decay challenge mode.",	4000000,[3,0,kaizoCookies.images.custImg]));Game.last.pool='prestige';Game.last.iconFunction=function(){return Game.EnchantedPermanentSlotIcon(3);};Game.last.activateFunction=function(){Game.AssignEnchantedPermanentSlot(3);};
			...
			this.achievements.push(new Game.Upgrade('Enchanted Permanent upgrade slot V',"Placing an upgrade in this slot will make its effects <b>permanent</b> across all playthroughs, <b>even in the Unshackled decay challenge mode</b>.<br>In addition, having this upgrade makes you start with <b>0.01</b> less acceleration for the Unshackled decay challenge mode.",	500000000,[4,0,kaizoCookies.images.custImg]));Game.last.pool='prestige';Game.last.iconFunction=function(){return Game.EnchantedPermanentSlotIcon(4);};Game.last.activateFunction=function(){Game.AssignEnchantedPermanentSlot(4);};
			...
			this.achievements.push(new Game.Upgrade('Illustrium fingernails', 'Clicking halts decay for <b>25%</b> longer.<q>This illustrious metal gleams with a teal-green light. It seems to be especially effective in stabilizing reality.</q>', 1e20, [0, 5, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Vegetable-oiled joints', 'Clicking halts decay for <b>25%</b> longer.<q>Reject chemistry, embrace nature.</q>', 1e22, [0, 6, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Ultraviolet obliteration', 'Clicking halts decay for <b>25%</b> longer.<q>The power of the sun imprinted on my hand...</q>', 1e28, [0, 7, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Method acting', 'Clicking halts decay for <b>25%</b> longer.<q>Oh no, my hands are on fire!</q>', 1e34, [0, 8, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Sinister glint', 'Clicking halts decay for <b>25%</b> longer.<q>...</q>', 1e40, [0, 9, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Future clicks', 'Clicking halts decay for <b>25%</b> longer.<q>In the future, clicking will be eternal.</q>', 1e46, [0, 10, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			this.achievements.push(new Game.Upgrade('Shanzhai glucosium', 'Clicking halts decay for <b>25%</b> longer.<q>Despite its off-brand status, it is somehow better than the original. Just for this one.</q>', 1e52, [0, 11, kaizoCookies.images.custImg])); decay.offBrandFingers.push(Game.last);
			for (let i in decay.offBrandFingers) {
			...
			this.achievements.push(new Game.Upgrade('Box of challenges', 'Unlocks <b>new challenges</b> for the <b>Unshackled decay</b> challenge mode.<br>This upgrade was unlocked after obtaining at least <b>'+Beautify(3e8)+'</b> total prestige levels.<q>It\'s full of fun!</q>', 1, [8, 0, kaizoCookies.images.custImg])); 
			Game.last.pool = 'prestige';
			Game.last.showIf = function() {
				return Game.prestige > 3e8;
			}
			Game.last.parents = [Game.Upgrades['Shimmering veil']];
			Game.PrestigeUpgrades.push(Game.last);
			Game.last.posX = 72; Game.last.posY = 961;

			this.achievements.push(new Game.Upgrade('Truck of challenges', 'Unlocks <b>new challenges</b> for the <b>Unshackled decay</b> challenge mode.<br>This upgrade was unlocked after obtaining at least <b>'+Beautify(3e12)+'</b> total prestige levels.<q>Is a truck really necessary for this...?</q>', 1, [10, 0, kaizoCookies.images.custImg])); 
			Game.last.pool = 'prestige';
			Game.last.showIf = function() {
				return Game.prestige > 3e12;
			}
			...
			this.achievements.push(new Game.Upgrade('Santaic doom', 'Wrinklers take <b>4%</b> more damage for each santa level past Elfling.<q>Filth begone!</q>', 5e14, [18, 9])); Game.last.order = 25001;
			...
			this.upgrades.push(new Game.Achievement('A light reward', 'Claim your first <b>shiny wrinkler soul</b>, unlocking the <b>Utenglobe</b>.', [10, 3, kaizoCookies.images.custImg])); Game.Achievements['A light reward'].order = 9900;
		...
		this.checkAchievements=function(){//Adding the unlock condition
			...
			if (Game.Has('Season switcher') && Game.cookiesEarned > Game.Upgrades['Santaic doom'].basePrice) { Game.Unlock('Santaic doom'); }
	...
	save: function(){
		...
		str += '/' + decay.saveUtenglobe() + '/' + decay.saveMaterialCounts() + '/' + decay.saveHaltValues();
        return str;
    },
	loadStr: '',
	toLoad: false,
